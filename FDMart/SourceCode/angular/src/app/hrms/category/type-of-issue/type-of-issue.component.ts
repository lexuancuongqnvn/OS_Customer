...
import { DepartmentService, Department_ENTITY, HRM_Project_Management_Task_Type_ENTITY, ProjectManagementService } from 'src/app/shared/service-proxies/api-shared';
import { AlertMessageComponent } from 'src/app/shared/sidenav/alert-message/alert-message.component';
import { EditPageState } from 'src/app/shared/ultilities/enum/edit-page-state';
import { IUiAction } from 'src/app/shared/ultilities/ui-action';

@Component({
  selector: 'app-type-of-issue',
  templateUrl: './type-of-issue.component.html',
  styleUrls: ['./type-of-issue.component.css']
})
export class TypeOfIssueComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {

  isEdit:boolean = false;
  tbName: string = 'HRM_Project_Management_Task_Type';
  ...
  listDepartment:HRM_Project_Management_Task_Type_ENTITY[] = [];
  ...
  confirmDelete() {
    var p = new HRM_Project_Management_Task_Type_ENTITY();
    p.code = this.idSelect;
    p.type = 'DELETE';
    this.projectManagementService.hRM_Project_Management_Task_Type_Actions(p).subscribe(
      (respond: any) => {
        if(respond['status'] == '0'){
  ...
  confirmEdit() {
    ...
    this.projectManagementService.hRM_Project_Management_Task_Type_Actions(this.InputModel).subscribe(
      (respond: any) => {
        if(respond['status'] == '0'){
          this.showMessageSuccess(respond['message'])
          this.UpdateView();
  ...
  onClickAcction(id: number, storedName: string, param: string, keyService: string, classForm: string): void {
    switch(classForm){
      case EditPageState.add:{
        this.InputModel =  new HRM_Project_Management_Task_Type_ENTITY();
        this.dialogAdd.open();
        setTimeout(() => {
