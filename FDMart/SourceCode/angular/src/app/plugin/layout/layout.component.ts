...
export class LayoutComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  onUpdate(item: any): void {
    ...
    input.block_type = this.selectedBlockBookId;
  ...
  { id: 1, name:'Khóa toàn bộ theo ngày/ Block all by date'},
  { id: 2, name:'Khóa Theo chi nhánh/ Block by branch'},
  { id: 3, name:'Khóa Theo chứng từ/ Block by voucher'},
];
...
public async checkAndLoadDataApp(isshowApp:boolean = true,item:any=null){
  ...
  async onLoadVoucherYear(){
    ...
    if(this.SYS_List_Voucher.length == 0 && this.selectedBlockBookId==3)
    {
      this.zoomDrawer()
      this.SYS_List_Voucher = await this.eRPCommonService.sYS_List_Voucher_Search(new SYS_Voucher_Year_ENTITY({company_code:''})).toPromise();
      this.dataSource = new DataSource({
        store: new ArrayStore({
          data: this.SYS_List_Voucher,
          key: 'code' // Specify the unique identifier field
