...
export class InputFilterComponent extends LayoutComponentBase implements OnInit, OnChanges, DoCheck {
  ...
  refresh() {
    if (this.tableName != undefined && this.colName != undefined) {
      ...
      try {
        ...
        if ((this.type == 6 || this.type == 7 || this.type == 8) && this.myFiles != undefined) {
          var key = this.getKey(this.tableName, this.colName, this.id);

          this.myFiles = this.objValueByID[key];
          if (this.objValueByID[key] == "") this.myFiles = [];

          if (this.myFiles == undefined || this.myFiles == null) this.myFiles = [];
        }
        if (this.type != 6 && this.type != 7 && this.type != 12 && this.type != 24 && this.type != 8)
          this.setValueEdit(this.key, this.value);
  ...
  onLoadCombobox(key: string, stored: string, tbName: string, colName: string, id: any) {
    ...
    input.type = 2;
    input.idEdit = parseInt(id);
    input.userID = this.appSession.user.id;
    if (this.value)
