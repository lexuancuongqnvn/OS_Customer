...
export class LayoutComponentBase {
    ...
    getStartEndDateInMonth(inputDate: moment.Moment = this.getFullVoucherDate): { startDate: moment.Moment; endDate: moment.Moment; }{
        ...
        let startDate = moment().set('year',currentYear).set('month',currentMonth - 1).set('date',1).set('hour',0).set('minute',0).set('second',1);
        let endDate = moment().set('year',currentYear).set('month',currentMonth).set('date',1).set('hour',23).set('minute',23).set('second',0);
