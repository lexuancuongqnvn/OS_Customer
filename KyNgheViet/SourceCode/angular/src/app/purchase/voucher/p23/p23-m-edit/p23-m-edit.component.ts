...
import { Accounting_VAT_Input_ENTITY, CAT_Tax_ENTITY, ERPCommonService, ERPCommon_ENTITY, P23_D_ENTITY, P23_M_ENTITY, PurchaseVoucherService, SalesVATService } from 'src/app/shared/service-proxies/api-shared';
import { EditPageState } from 'src/app/shared/ultilities/enum/edit-page-state';
import { IUiAction } from 'src/app/shared/ultilities/ui-action';

@Component({
  selector: 'app-p23-m-edit',
  templateUrl: './p23-m-edit.component.html',
  styleUrls: ['./p23-m-edit.component.css']
})
export class P23MEditComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  InputMaster:P23_M_ENTITY=new P23_M_ENTITY();
  ...
  onDelete(item: any): void {
    this.purchaseVoucherService.p23_M_Delete(this.InputMaster).subscribe(res=>{
      this.showMessage(res.message,res.status);
      this.navigatePassParam('purchase/import-invoice-with-warehouse-receipt',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new P23_M_ENTITY({}),this.tbName)
    })
  }
  ...
  onClickAcctionResponse(e:any): void {
    switch(e.classForm){
      ...
      case EditPageState.edit:{
        this.navigatePassParam('purchase/import-invoice-with-warehouse-receipt-edit',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new P23_M_ENTITY({}),this.tbName)
        break;
      }
      case EditPageState.save:{
        if(this.InputMaster.p23_D){
          const i42D: P23_D_ENTITY[] = this.InputMaster.p23_D.map((obj: any) => {
            let item = new P23_D_ENTITY();
            for (const [key, value] of Object.entries(obj)) {
              item[key] = value;
            }
            return item;
          });
          this.InputMaster.p23_D = i42D;
        }
        ...
        if(!this.InputMaster.code){
          this.purchaseVoucherService.p23_M_Insert(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status);
            if(res.status == 0){
              this.InputMaster.code = res.ref_code;
              this.navigatePassParam('purchase/import-invoice-with-warehouse-receipt-view-detail',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new P23_M_ENTITY({}),this.tbName)
            }
          })
        }else{
          this.purchaseVoucherService.p23_M_Update(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status);
            if(res.status == 0)this.navigatePassParam('purchase/import-invoice-with-warehouse-receipt-view-detail',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new P23_M_ENTITY({}),this.tbName)
  ...
  handleValueChanged(event: any) {
    ...
    else if(event.dataField == 'p23_D' && this.InputMaster.p23_D){
      this.updateVATIN();
    }
    this.InputMaster[event.dataField]= event.value;
  ...
  updateVATIN(e:any = undefined){
    if(this.InputMaster.is_tax){
      for(var i = 0 ; i < this.InputMaster.p23_D.length ; i ++){
        let s33D = this.InputMaster.p23_D[i];
  ...
  async caculateSumMoney(){
    if (this.InputMaster.exchange_rate == 1){
      try{
        this.InputMaster.total_money_goods = this.InputMaster.p23_D.reduce((accumulator, currentObject) => {
          return accumulator + currentObject.arise;
        }, 0);
        this.InputMaster.tax_import_money = this.InputMaster.p23_D.reduce((accumulator, currentObject) => {
          return accumulator + currentObject.tax_import;
        }, 0);
      ...
      try{
        this.InputMaster.total_money_goods_fc = this.InputMaster.p23_D.reduce((accumulator, currentObject) => {
          return accumulator + currentObject.arise_fc;
        }, 0);
        this.InputMaster.tax_import_money_fc = this.InputMaster.p23_D.reduce((accumulator, currentObject) => {
          return accumulator + currentObject.tax_import_fc;
        }, 0);
        this.InputMaster.tax_money_fc = this.formatDefaultNumber(this.InputMaster.total_money_goods_fc * (this.getTax.tax/100));
        this.InputMaster.total_money_fc = this.formatDefaultNumber(this.InputMaster.total_money_goods_fc + this.InputMaster.tax_money_fc + this.InputMaster.cost_fc+ this.InputMaster.tax_import_money_fc);
  ...
  onLoadData(){
    this.purchaseVoucherService.p23_M_Search(new P23_M_ENTITY({
      // voucher_date:moment(this.InputMaster.voucher_date).utc(true),
      code:this.InputMaster.code,
      voucher_code:this.appSession.getVoucherCode
    }) as P23_M_ENTITY).subscribe((resonse:P23_M_ENTITY[])=>{
      if(resonse[0])
