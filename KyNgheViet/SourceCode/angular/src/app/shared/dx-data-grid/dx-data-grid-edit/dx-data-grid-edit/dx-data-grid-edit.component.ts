...
export class DxDataGridEditComponent extends LayoutComponentBase implements OnInit   {
  ...
  onClosedDropDownOption(e: any, cell: any, items: any, key: any, row: any) {
    ...
    if (!this.dataSource[cell.row.dataIndex]) {
      this.dataSource[cell.row.dataIndex] = {};
      // this.dataSource[cell.row.dataIndex][this.keyExpr] = 'NEW-'+Math.random()+(new Date()).getTime();
    }
    this.dataSource[cell.row.dataIndex][cell.column.dataField] = this.arrayValueDropDown;
    ...
    try {
      ...
      arrDefault.forEach(element => {
        let objRow = this.dataSource[cell.row.dataIndex];
        ...
        try {
          ...
          this.dataSource[cell.row.dataIndex][element.columN_NAME] = result;
          this.dataGrid.instance.refresh();
          this.HandleValueChangedOutput.emit({ rowIndex: cell.row.dataIndex, dataField: element.columN_NAME, row: element, value: result, dataSource: this.dataSource });
    ...
    try {
      ...
      arrDefault2.forEach(element => {
        ...
        if (this.dataSource[cell.row.dataIndex][element.columN_NAME] === undefined || this.dataSource[cell.row.dataIndex][element.columN_NAME] === null) {
    ...
    this.HandleValueChangedOutput.emit({ rowIndex: cell.row.dataIndex, dataField: cell.column.dataField, row: row, value: this.dataSource[cell.row.dataIndex][cell.column.dataField], dataSource: this.dataSource });
  ...
  onSavTagboxMultiOption(e: any, cell: any, items: any, key: any, row: any) {
    ...
    this.dataSource[cell.row.dataIndex][cell.column.dataField] = e.value;
    this.arrayValueDropDown = [];
    this.isDropBoxOpened = false;
    try {
      var arrDefault = this.genRowTableDetail.filter(r => r.defaulT_VALUE != null && r.defaulT_VALUE != '' && r.columN_NAME !== cell.column.dataField && r.defaulT_VALUE.startsWith('='));
  ...
  onSaveDropDownOption(e: any, cell: any, items: any, key: any, row: any) {
    ...
    try {
      let objRow = this.dataSource[cell.row.dataIndex];
      if (objRow[row.columN_NAME] && this.genRowTableDetail.filter(r => r.shoW_REFERENCE == true && r.columN_NAME === row.columN_NAME).length > 0) {
        var arrDefault3 = this.genRowTableDetail.filter(r => r.columN_NAME!=row.columN_NAME && (r.editortype === 'Multirow' || r.editortype === 'dxTagBox' || r.editortype === 'dxSelectBox') && r.reference.includes('{') && r.reference.includes('}'));
        arrDefault3.forEach(async element => {
          let itemReload = await this.referenceService.reference_V3_Search(new REFERENCE_V3_Param({
            reference: element.reference,
            master_data: this.checkAndConvertToObject(objRow)
  ...
  RenderMomentUTC(){
    for(var i = 0 ; i < this.dataSource.length ; i++){
      ...
      for (const [key, value] of Object.entries(record)) {
        ...
        if (value instanceof moment && value) {
          var v = value as moment.Moment;
          if(!v.isUTC()){
            //v = moment(v).utc(true);
            var row = this.genRowTableDetail.find(r=>r.columN_NAME == key);
            this.HandleValueChangedOutput.emit({ rowIndex: i, dataField: key, row: row, value: v, dataSource: this.dataSource });
          }
        } 
      };
  ...
  cellTemplateDate5(container, options, e) {
    try {
      ...
      const text = options.value && moment(options.value._d).format(format);
      container.textContent = text || noBreakSpace;
      container.title = text;
      container.style.textAlign = "right"
    } catch (error) {
  ...
  async setEditorOptions(data: SYS_GenRowTable_Detail[]): Promise<void> {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      switch (dt.editortype) {
        case 'dxSelectBox':
          let items1 = this.referenceService.reference_V2_Search(new REFERENCE_ENTITY({
              code: dt.code,
              userID: this.appSession.user.id,
            ...
            listResquest.push(items1);
          break;
        case 'dxTagBox':
            let items = this.referenceService.reference_V2_Search(new REFERENCE_ENTITY({
                code: dt.code,
                userID: this.appSession.user.id,
                code_master: this.InputMaster.code,
                user: new User({
        ...
        case 'dxTagBox':
          var key = dt.reference;
            ...
            if (!items) {
              ...
              if (!items) {
                items = await this.referenceService
                  .reference_V2_Search(
                    new REFERENCE_ENTITY({
                      code: dt.code,
                      userID: this.appSession.user.id,
                      code_master: this.InputMaster.code,
                      user: new User({
  ...
  handleValueChangedDate(e: any, cell: any, row: any) {
    this.dataSource[cell.row.dataIndex][cell.column.dataField] = e;
    this.HandleValueChangedOutput.emit({ rowIndex: cell.row.dataIndex, dataField: cell.column.dataField, row: row, value: e, dataSource: this.dataSource });
  }
  ...
  onSelectRowGridDropdown(event: any, cell: any, column: SYS_GenRowTable_Detail,row: SYS_GenRowTable_Detail = new SYS_GenRowTable_Detail()) {
    this.arrayValueDropDown = event;
    this.selectedRowsGridDataOutput.emit({event:event,cell:cell,row:row});
  }
  ...
  AddRecordCombobox(event: any, cell: any, row: SYS_GenRowTable_Detail,cellInfo:any,items:any,i:number,valueexpr) {
    this.OnClickActionForm.emit({event:event,cell:cell,row:row,cellInfo:cellInfo,items:items,i:i})
  }
}
