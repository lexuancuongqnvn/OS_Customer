...
export class InputEditComponent extends LayoutComponentBase implements OnInit, OnChanges, DoCheck {
  ...
  ngDoCheck(): void {
    if (this.isChangeDetected()) {
      this.refresh();
      if (Object.keys(this.objValueByID).length == this.getCountItems() || Object.keys(this.objValueByID).length > 1000)
    ...
    if (this.tableName != undefined && this.colName != undefined && this.id != undefined) {
      ...
      try {
        ...
        if ((this.type == 6 || this.type == 7 || this.type == 8) && this.myFiles != undefined) {
          ...
          if (!isFirst) {
            this.value = this.myFiles.filter(e => e['ref_MasterID'] == this.codeEdit && e['colName'] == this.colName && e['tbName'] == this.tableName);
            if (this.objValueByID[this.key])
        ...
        // console.log('Sau ' + this.key)
        // console.log(this.objValueByID[this.key])
  ...
  public onChangeValueFile(val: any, id: number) {
    this.refValue = { id: id, v: val, key: val['key'] }
