...
@Component({
  selector: 'app-form-edit-v2-for-popup',
  templateUrl: './form-edit-v2-for-popup.component.html',
  styleUrls: ['./form-edit-v2-for-popup.component.css']
})
export class FormEditV2ForPopupComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  onLoadGenRowTableDetail() {
    ...
    if(!cache){
      this.genRowTableService.sYS_GenRowTable_Search({
        ...
        this.setItemWithExpiry(key,this.genRowTable,3600);
        this.genRowTableDetail = this.genRowTable.syS_GenRowTable_Detail;
        this.setEditorOptions(this.genRowTableDetail);
        this.UpdateView();
        this.UnBlockUI();
  ...
  OnClickActionForm(e: any) {
    // this.dialogForm.open()
    this.showMessageWarning('Không thể mở tiện ích khi đang mở tiện ích hiện tại')
  }
