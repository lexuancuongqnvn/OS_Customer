...
import { AccountService, AcctionService, POS_Product_Unit_ENTITY, ProductService, ReferenceService } from 'src/app/shared/service-proxies/api-shared';
...
export class UnitComponent  extends LayoutComponentBase implements OnInit, IUiAction<any> {

  @ViewChild('gridMenuList') gridMenuList: JqgridListComponent;
  @ViewChild('gridMenuEdit') gridMenuEdit: JqgridEditComponent<POS_Product_Unit_ENTITY>;
  ...
  tbName: string = 'POS_Product_Unit';
  CurrenFrom:string = EditPageState.view;
  InputMaster: POS_Product_Unit_ENTITY[] = [];
  InputModel: POS_Product_Unit_ENTITY = new POS_Product_Unit_ENTITY();
  filterInput:POS_Product_Unit_ENTITY = new POS_Product_Unit_ENTITY();
  isEdit:boolean = false;

  col_model:any[] = [
    { label: 'Mã', name: 'code', key: true, width: 10 , hidden: true},
    { label: 'Tên đơn vị', name: 'name', width: 15 }
  ]
  constructor(
  ...
  onClickAcction(id: number, storedName: string, param: string, keyService: string, classForm: string): void {
    switch(classForm){
      case EditPageState.add:{
        this.isEdit = true;
        this.InputModel = new POS_Product_Unit_ENTITY();
        setTimeout(() => {
          this.sidenavAddEdit.title = "Thêm mới đơn vị";
      ...
      case EditPageState.edit:{
        ...
        this.productService.pOS_Product_Unit_Actions(this.filterInput).subscribe((respond)=>{
          this.InputModel = respond as POS_Product_Unit_ENTITY;
          this.sidenavAddEdit.open();
          this.toolbarEdit.setUiAction(this);
          this.UpdateView();
        },
      ...
      case EditPageState.save:{
        if(!this.InputModel.code){
          ...
          this.productService.pOS_Product_Unit_Actions(this.InputModel).subscribe((respond)=>{
            if(respond['status'] == '0')
            {
  ...
  LoadData() {
    ...
    this.productService.pOS_Product_Unit_Search(this.filterInput).subscribe(
      (data: any) => {
