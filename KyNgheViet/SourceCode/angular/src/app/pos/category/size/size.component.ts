...
import { POS_Product_Size_ENTITY, ProductService, ReferenceService } from 'src/app/shared/service-proxies/api-shared';
...
export class SizeComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {

  @ViewChild('gridMenuList') gridMenuList: JqgridListComponent;
  @ViewChild('gridMenuEdit') gridMenuEdit: JqgridEditComponent<POS_Product_Size_ENTITY>;
  ...
  tbName: string = 'POS_Product_Size';
  CurrenFrom:string = EditPageState.view;
  InputMaster: POS_Product_Size_ENTITY[] = [];
  InputModel: POS_Product_Size_ENTITY = new POS_Product_Size_ENTITY();
  filterInput:POS_Product_Size_ENTITY = new POS_Product_Size_ENTITY();
  ...
  onClickAcction(id: number, storedName: string, param: string, keyService: string, classForm: string): void {
    switch(classForm){
      case EditPageState.add:{
        this.isEdit = true;
        this.InputModel = new POS_Product_Size_ENTITY();
        setTimeout(() => {
      ...
      case EditPageState.edit:{
        ...
        this.productService.pOS_Product_Size_Actions(this.filterInput).subscribe((respond)=>{
          this.InputModel = respond as POS_Product_Size_ENTITY;
          this.sidenavAddEdit.open();
          this.toolbarEdit.setUiAction(this);
          this.UpdateView();
        },
      ...
      case EditPageState.save:{
        if(!this.InputModel.code){
          ...
          this.productService.pOS_Product_Size_Actions(this.InputModel).subscribe((respond)=>{
            if(respond['status'] == '0')
            {
  ...
  LoadData() {
    ...
    this.productService.pOS_Product_Size_Search(this.filterInput).subscribe(
      (data: any) => {
