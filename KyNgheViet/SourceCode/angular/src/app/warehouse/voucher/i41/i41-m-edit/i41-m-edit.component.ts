...
@Component({
  selector: 'app-i41-m-edit',
  templateUrl: './i41-m-edit.component.html',
  styleUrls: ['./i41-m-edit.component.css']
})
export class I41MEditComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  onDelete(item: any): void {
    this.wMSVoucherService.i41_M_Delete(this.InputMaster).subscribe(res=>{
      this.showMessage(res.message,res.status);
      this.navigatePassParam('warehouse/finished-goods-receipt-from-production',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new I41_M_ENTITY({}),this.tbName)
    })
  }
  ...
  onClickAcctionResponse(e:any): void {
    switch(e.classForm){
      case EditPageState.add:{
        this.navigatePassParam('warehouse/finished-goods-receipt-from-production-add',[['code','']],new I41_M_ENTITY({}),this.tbName)
        break;
      }
      case EditPageState.edit:{
        this.navigatePassParam('warehouse/finished-goods-receipt-from-production-edit',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new I41_M_ENTITY({}),this.tbName)
        break;
      }
      case EditPageState.save:{
        if(this.InputMaster.i41_D){
          const i42D: I41_D_ENTITY[] = this.InputMaster.i41_D.map((obj: any) => {
            let item = new I41_D_ENTITY();
            for (const [key, value] of Object.entries(obj)) {
              item[key] = value;
            }
            return item;
          });
          this.InputMaster.i41_D = i42D;
        }
        
        if(!this.InputMaster.code){
          this.wMSVoucherService.i41_M_Insert(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status);
            if(res.status == 0){
              this.InputMaster.code = res.ref_code;
              this.navigatePassParam('warehouse/finished-goods-receipt-from-production-view-detail',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new I41_M_ENTITY({}),this.tbName)
            }
          })
        }else{
          this.wMSVoucherService.i41_M_Update(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status);
            if(res.status == 0)this.navigatePassParam('warehouse/finished-goods-receipt-from-production-view-detail',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new I41_M_ENTITY({}),this.tbName)
  ...
  async caculateSumMoney(){
    try{
      this.InputMaster.total_money = this.InputMaster.i41_D.reduce((accumulator, currentObject) => {
  ...
  onLoadData(){
    this.wMSVoucherService.i41_M_Search(new I41_M_ENTITY({
      //voucher_date:moment(this.InputMaster.voucher_date).utc(true),
      code:this.InputMaster.code,
      voucher_code:this.appSession.getVoucherCode
    }) as I41_M_ENTITY).subscribe((resonse:I41_M_ENTITY[])=>{
      if(resonse[0])
