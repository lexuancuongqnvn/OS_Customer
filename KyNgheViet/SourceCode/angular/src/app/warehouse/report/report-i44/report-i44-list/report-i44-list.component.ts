...
@Component({
  selector: 'app-report-i44-list',
  templateUrl: './report-i44-list.component.html',
  styleUrls: ['./report-i44-list.component.css']
})
export class ReportI44ListComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  onClickAcction(id: number, storedName: string, param: string, keyService: string, classForm: string): void {
    switch(classForm){
      case EditPageState.add:{
        this.navigatePassParam('warehouse/report-i44-add',[['code',this.idSelect]],new WMS_Report_I44_ENTITY({}),this.tbName)
        break;
      }
      case EditPageState.edit:{
        this.navigatePassParam('warehouse/report-i44-edit',[['code',this.idSelect]],new WMS_Report_I44_ENTITY({}),this.tbName)
        break;
      }
  ...
  onLoadData(){
    this.BlockUI();
    this.wMSReportService.wMS_Report_I44_Search({
      ...this.filterInput
    } as WMS_Report_I44_ENTITY).subscribe((res:WMS_Report_I44_ENTITY[])=>{
      this.listData = res;
