...
import { AccountService, AcctionService, ReferenceService, SYS_Account_Group, WarehouseService, WMS_Warehouse_Goods_Issue_ENTITY } from 'src/app/shared/service-proxies/api-shared';
...
export class GoodsIssueComponent  extends LayoutComponentBase implements OnInit, IUiAction<any> {

  @ViewChild('gridMenuList') gridMenuList: JqgridListComponent;
  @ViewChild('gridMenuEdit') gridMenuEdit: JqgridEditComponent<WMS_Warehouse_Goods_Issue_ENTITY>;
  ...
  InputMaster: WMS_Warehouse_Goods_Issue_ENTITY[] = [];
  sYS_Menu_Sub: WMS_Warehouse_Goods_Issue_ENTITY[] = [];
  InputModel: WMS_Warehouse_Goods_Issue_ENTITY = new WMS_Warehouse_Goods_Issue_ENTITY();
  filterInput:WMS_Warehouse_Goods_Issue_ENTITY = new WMS_Warehouse_Goods_Issue_ENTITY();
  ...
  onClickAcction(id: number, storedName: string, param: string, keyService: string, classForm: string): void {
    switch(classForm){
      case EditPageState.add:{
        this.isEdit = true;
        this.InputModel = new WMS_Warehouse_Goods_Issue_ENTITY();
        setTimeout(() => {
      ...
      case EditPageState.edit:{
        this.isEdit = true;
        this.InputModel = new WMS_Warehouse_Goods_Issue_ENTITY();
        this.warehouseService.wMS_Warehouse_Goods_Issue_Bycode(this.idSelect).subscribe((respond)=>{
          this.InputModel = respond[0];
          this.sidenavAddEdit.open();
          this.toolbarEdit.setUiAction(this);
          this.UpdateView();
          this.gridMenuEdit.setData(this.InputModel.wMS_Warehouse_Export_Details);
          this.UpdateView();
        },
        (err) => {
            if (err.status == 401) {
      ...
      case EditPageState.save:{
        try{
          ...
          this.InputModel.wMS_Warehouse_Export_Details = [];
          var wMS_Warehouse_Export_Details = this.gridMenuEdit.allData;
          if(wMS_Warehouse_Export_Details.length>0)
          {
            for(var i = 0 ; i < wMS_Warehouse_Export_Details.length ; i++)
            {
              var newrow = new WMS_Warehouse_Goods_Issue_ENTITY();
              for (const [key, value] of Object.entries(wMS_Warehouse_Export_Details[i])) {
                newrow[key] = wMS_Warehouse_Export_Details[i][key];
              }
              newrow['userID'] = this.appSession.user.id;
              this.InputModel.wMS_Warehouse_Export_Details.push(newrow);
            }
          }
        }catch{}
        // this.InputModel = SYS_Menu['toJSON'](this.InputModel);
        if(!this.InputModel.code){
          this.warehouseService.wMS_Warehouse_Goods_Issue_Insert(this.InputModel).subscribe((respond)=>{
            if(respond['status'] == '0')
            {
          ...
          this.warehouseService.wMS_Warehouse_Goods_Issue_Update(this.InputModel).subscribe((respond)=>{
            if(respond['status'] == '0')
            {
              this.showMessageSuccess(respond['message']);
              this.sidenavAddEdit.close();
  ...
  initCombobox(){
    ...
    this.accountService.sYS_Account_Group_Search(p).subscribe(
      (data: any) => {
        this.col_model_edit[9].editoptions.value=data;
        this.UpdateView();
      },
      (err) => this.UnBlockUI(),
      () => {
  ...
  LoadData() {
    this.BlockUI();
    this.warehouseService.wMS_Warehouse_Goods_Issue_Search(this.filterInput).subscribe(
      (data: any) => {
