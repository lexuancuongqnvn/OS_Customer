...
@Component({
  selector: 'app-employee-salary-by-attendance-report-v2',
  templateUrl: './employee-salary-by-attendance-report-v2.component.html',
  styleUrls: ['./employee-salary-by-attendance-report-v2.component.css']
})
export class EmployeeSalaryByAttendanceReportV2Component extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  listEmployeeCodeExport:string[]=[];
  table:string = '';
  tableMarkup: SafeHtml;
  tbName: string = 'HRM_Report_Attendance';
  tbNameSalary: string = 'HRM_Employee_Report_Salary_Employee';
  CurrenFrom:string = EditPageState.view;
  isEdit:boolean = false;
  isAll:boolean = true;
  @ViewChild('toolbar') toolbar: ToolbarComponent;
  @ViewChild('SelectTypeSalary') SelectTypeSalary: InputSelectComponentV2;
  ...
  onClickAcction(id: number, storedName: string, param: string, keyService: string, classForm: string): void {
    switch(classForm){
      ...
      case 'export_salary':{
        if(!this.listEmployeeExportSalary || !this.listEmployeeExportSalary.list_salary_employees || this.listEmployeeExportSalary.list_salary_employees.length == 0) {
      ...
      case 'download':{
        if(this.listEmployeeExportSalary.list_salary_employees.filter(emp=>emp.is_use == true).length ==0)
          this.showMessageError('Vui lòng chọn nhân viên để tải xuống'); 
        else this.ExportSalary()
        break;
      }
      default:break;
    }
  ...
  ExportSalary(){
    if(this.listEmployeeExportSalary.list_salary_employees.filter(emp=>emp.is_use == true).length ==0)
    {
      this.showMessageError('Vui lòng chọn nhân viên để tải xuống');return;
    }
    ...
    this.BlockUI('Đang chuẩn bị tải xuống...');
    this.employeeService.hRM_Employee_Report_Salary_v2_Search(this.listEmployeeExportSalary).subscribe((res:HRM_Employee_Report_Salary_ENTITY)=>{
      ...
      this.timeSheetService.hRM_Employee_Salary_By_Report_Attendance_v2_Search(this.filterInput).subscribe(data=>{
        ...
        try{
          ...
          for(var i = 0 ; i < this.table_employee.length ; i++){
              ...
              for (let [key, value] of Object.entries(this.table_day)) {
                ...
                if(key != 'account_id' && key != 'id' && key != 'employee_code'){
                 ...
                 // console.log(val+'=>'+total)
                  this.tbody_td += `<td class="tr-cell-attendance-data">${_v.toString()=='0'?'-':
                  (_v == 1?'1':_v.toFixed(3))
                }</td>`;
                }
              }
            ...
            <table class="table timesheet-segment-table">
                <thead>
                  <tr>
                    ...
                    <th scope="col">Công bị trừ(${total_work_day_minus.toFixed(3)})</th>
                  </tr>
                </thead>
                <tbody>
                  ${tr_log} 
            ...
            let total_salary_day = (total_salary/sum_standard);
            let total_income = (total_salary_day * t)-(total_salary_day * total_work_day_minus);
            this.tbody_td += `<td class="tr-cell-attendance-data">${(total_income).toFixed(3).replace(/\d(?=(\d{3})+\.)/g, '$&,').replace('.00','')} đ </td>`;
            try{
              ...
              temp.sum_standard=sum_standard;
              temp.work_shifts=employee.work_shifts;
              temp.is_use=false;
              temp.sum_salary_by_work_day=total_income;
              temp.sum_work_day_practical=(t - total_work_day_minus);
              temp.total_work_day_minus=total_work_day_minus;
              temp.total_salary = total_salary;
              temp.name = employee.name;
              temp.branch_name = employee.branch_name;
              this.listEmployeeCodeExport.push(employee.code)
          ...
          this.table = `<table class="table table-bordered table-attendance-report">
                          <thead>
                              <tr>
                                  <th colspan="4" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Thông tin nhân viên</th>
                                  <th colspan="${(colspan_table_day+1)}" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Ngày công</th>
                                  ...
                                  <th colspan="2" class="m-w-100" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Phiếu nghỉ phép</th>
                                  <th colspan="2" class="m-w-150" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Phiếu đi muộn,về sớm</th>
                                  <th colspan="1" class="m-w-100" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Phiếu công tác</th>
                                  <th colspan="1" class="m-w-100" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Phiếu bù công</th>
                                  <th colspan="${this.listOverTime.length}" class="m-w-100" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Phiếu tăng ca</th>
                                  <th colspan="1" class="m-w-150" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Nghỉ lễ</th>
                                  <th colspan="1" class="m-w-150" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Tổng công hưởng lương</th>
                                  <th colspan="1" class="m-w-150" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Tổng năng suất</th>
                                  <th colspan="1" class="m-w-150" style="text-align: center;background-color: #164478;font-weight: 600;;color: white;">Tổng lương thành tiền ngày công thực tế</th>
                              </tr>
                          </thead>
                          <tbody>
                            ${this.thead_th}
  ...
  ngOnInit(): void {
    ...
    this.filterInput.to_date = moment().endOf('month');

    // this.onLoad();
    this.setAcction();
    setTimeout(() => {
      this.SelectTypeSalary.setList(this.listType);
      renderChartWT([]);
    }, 100);
  }
  callEventClick(){
  ...
  selectEmployee(e:any){
    this.listEmployeeExportSalary.list_salary_employees.map(item=>item.is_use = e.includes(item.code))
  }
  OnCloseExport(e:any){
    this.setCurrenFrom('View');
    this.toolbar.setUiAction(this);
  }
}
