...
import { AccountService, AcctionService, SYSCommonService, SYS_Account_Group, SYS_Account_Group_Permission, SYS_List_App_ENTITY } from 'src/app/shared/service-proxies/api-shared';
...
export class SysListAppComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  @ViewChild('SelectUserPicker') SelectUserPicker: InputSelectComponentV2;
  ...
  listAccount:SYS_Account_Group_Permission[] = [];
  dataGrid: any[] = [];
  listTbName: any[] = [];
  ...
  initCombobox(){
    ...
    this.accountService.sYS_Account_Info_Permission_Search().subscribe(
      (data: any) => {
        if (data == null) return;
        this.listAccount = data;
        this.SelectUserPicker.setList(this.listAccount);
        this.UpdateView()
      },
      (err) => this.UnBlockUI(),
      () => {
