...
export class ObAccountEditComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  onClickAcctionResponse(e:any): void {
    switch(e.classForm){
      ...
      case EditPageState.edit:{
        this.navigatePassParam('con/ob-accounts-edit',[['code',this.InputMaster.code]],new OB_Account_ENTITY(),this.tbName);
        break;
      }
      case EditPageState.save:{
        if(!this.InputMaster.code){//chỗ này dùng code nha
          ...
          this.oBPurchaseService.oB_Account_Insert(this.InputMaster).subscribe(res=>{
            ...
            if(res.status == 0)
             this.navigatePassParam('con/ob-accounts-view-detail',[['code',res.ref_code]],new OB_Account_ENTITY(),this.tbName);
          })
        }else{
          //this.InputMaster.type = 'UPDATE'
          this.oBPurchaseService.oB_Account_Update(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status)
            if(res.status == 0) this.navigatePassParam('con/ob-accounts-view-detail',[['code',res.ref_code]],new OB_Account_ENTITY(),this.tbName);
          })
        }
        break;
      }
  ...
  confirmDelete(){
    ...
    this.oBPurchaseService.oB_Account_Delete(this.InputMaster).subscribe(res=>{
      this.showMessage(res.message,res.status);
      if(res.status == 0)
             this.navigatePassParam('con/ob-accounts',[['code',res.ref_code]],new OB_Account_ENTITY(),this.tbName);
    })
  }
