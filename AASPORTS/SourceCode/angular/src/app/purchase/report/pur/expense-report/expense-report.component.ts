...
import { PUR_Aggregate_Cost_Coupon_Print_ENTITY, PUR_Expense_Report_ENTITY, PurchaseReportService } from 'src/app/shared/service-proxies/api-shared';
...
export class ExpenseReportComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  filterInput:PUR_Expense_Report_ENTITY=new PUR_Expense_Report_ENTITY();
  rowSelected:PUR_Expense_Report_ENTITY=new PUR_Expense_Report_ENTITY();
  listData:PUR_Expense_Report_ENTITY[]=[];
  tbName:string = '';
  ...
  onClickAcction(id: number, storedName: string, param: string, keyService: string, classForm: string): void {
    switch(classForm){
      case EditPageState.add:{
        this.navigatePassParam('warehouse/opening-balance-input-output-inventory-add',[['code',this.idSelect]],new PUR_Expense_Report_ENTITY({}),this.tbName)
        break;
      }
      case EditPageState.edit:{
        this.navigatePassParam('warehouse/opening-balance-input-output-inventory-edit',[['code',this.idSelect]],new PUR_Expense_Report_ENTITY({}),this.tbName)
        break;
      }
  ...
  onLoadData(){
    ...
    this.BlockUI();
    this.purchaseReportService.pUR_Expense_Report_Search({
      ...this.filterInput
    } as PUR_Expense_Report_ENTITY).subscribe((res:PUR_Expense_Report_ENTITY[])=>{
      this.listData = res;
