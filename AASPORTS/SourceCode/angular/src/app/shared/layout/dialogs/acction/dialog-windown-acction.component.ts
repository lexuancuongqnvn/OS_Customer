...
@Component({
    selector: 'dialog-windown-acction',
    template: `
        <button kendoButton *ngIf="!opened" (click)="open()" style="z-index: -111;"></button>
        <kendo-window title="{{title}}" *ngIf="opened" (close)="close('cancel')" [minWidth]="250" [width]="widthScreen > width ? width : widthScreen" [maxHiegh]="hieghtScreen">
...
export class DialogWindownAcctionComponent {
    public opened = false;
