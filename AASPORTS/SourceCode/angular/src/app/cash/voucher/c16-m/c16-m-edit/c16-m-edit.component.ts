...
@Component({
  selector: 'app-c16-m-edit',
  templateUrl: './c16-m-edit.component.html',
  styleUrls: ['./c16-m-edit.component.css']
})
export class C16MEditComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  onDelete(item: any): void {
    this.cashVoucherService.c16_M_Delete(this.InputMaster).subscribe(res=>{
      this.showMessage(res.message,res.status);
      this.navigatePassParam('cash/debit-note',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new C16_M_ENTITY({}),this.tbName)
    })
  }
  ...
  onClickAcctionResponse(e:any): void {
    switch(e.classForm){
      case EditPageState.add:{
        this.navigatePassParam('cash/debit-note-add',[['code','']],new C16_M_ENTITY({}),this.tbName)

        break;
      }
      case EditPageState.edit:{
        this.navigatePassParam('cash/debit-note-edit',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new C16_M_ENTITY({}),this.tbName)
        break;
      }
      case EditPageState.save:{
        if(this.InputMaster.c16_d){
          const D: C16_D_ENTITY[] = this.InputMaster.c16_d.map((obj: any) => {
            let item = new C16_D_ENTITY();
            for (const [key, value] of Object.entries(obj)) {
              item[key] = value;
            }
            return item;
          });
          this.InputMaster.c16_d = D;
        }
       
        if(!this.InputMaster.code){
          this.cashVoucherService.c16_M_Insert(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status);
            if(res.status == 0){
              this.InputMaster.code = res.ref_code;
              this.navigatePassParam('cash/debit-note-view-detail',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new C16_M_ENTITY({}),this.tbName)
            }
          })
        }else{
          this.cashVoucherService.c16_M_Update(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status);
            if(res.status == 0)this.navigatePassParam('cash/debit-note-view-detail',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new C16_M_ENTITY({}),this.tbName)
  ...
  async caculateSumMoney(){
    try{
      this.InputMaster.arise = this.InputMaster.c16_d.reduce((accumulator, currentObject) => {
  ...
  onLoadData(){
    this.cashVoucherService.c16_M_Search(new C16_M_ENTITY({
      // voucher_date:moment(this.InputMaster.voucher_date).utc(true),
      code:this.InputMaster.code,
      voucher_code:this.appSession.getVoucherCode
    }) as C16_M_ENTITY).subscribe((resonse:C16_M_ENTITY[])=>{
      if(resonse[0])
