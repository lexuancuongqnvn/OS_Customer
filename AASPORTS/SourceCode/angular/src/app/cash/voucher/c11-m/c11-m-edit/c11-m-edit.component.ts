...
@Component({
  selector: 'app-c11-m-edit',
  templateUrl: './c11-m-edit.component.html',
  styleUrls: ['./c11-m-edit.component.css']
})
export class C11MEditComponent extends LayoutComponentBase implements OnInit, IUiAction<any> {
  ...
  onDelete(item: any): void {
    this.cashVoucherService.c11_M_Delete(this.InputMaster).subscribe(res=>{
      this.showMessage(res.message,res.status);
      this.navigatePassParam('cash/receipt',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new C11_M_ENTITY({}),this.tbName)
    })
  }
  ...
  onClickAcctionResponse(e:any): void {
    switch(e.classForm){
      case EditPageState.add:{
        this.navigatePassParam('cash/receipt-add',[['code','']],new C11_M_ENTITY({}),this.tbName)

        break;
      }
      case EditPageState.edit:{
        this.navigatePassParam('cash/receipt-edit',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new C11_M_ENTITY({}),this.tbName)
        break;
      }
      case EditPageState.save:{
        if(this.InputMaster.c11_d){
          const D: C11_D_ENTITY[] = this.InputMaster.c11_d.map((obj: any) => {
            let item = new C11_D_ENTITY();
            for (const [key, value] of Object.entries(obj)) {
              item[key] = value;
            }
            return item;
          });
          this.InputMaster.c11_d = D;
        }
       
        if(!this.InputMaster.code){
          this.cashVoucherService.c11_M_Insert(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status);
            if(res.status == 0){
              this.InputMaster.code = res.ref_code;
              this.navigatePassParam('cash/receipt-view-detail',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new C11_M_ENTITY({}),this.tbName)
            }
          })
        }else{
          this.cashVoucherService.c11_M_Update(this.InputMaster).subscribe(res=>{
            this.showMessage(res.message,res.status);
            if(res.status == 0)this.navigatePassParam('cash/receipt-view-detail',[['code',this.InputMaster.code],['voucher_date',this.InputMaster.voucher_date.format('YYYY-MM-DD')]],new C11_M_ENTITY({}),this.tbName)
  ...
  onLoadData(){
    this.cashVoucherService.c11_M_Search(new C11_M_ENTITY({
      // voucher_date:moment(this.InputMaster.voucher_date).utc(true),
      code:this.InputMaster.code,
      voucher_code:this.appSession.getVoucherCode
    }) as C11_M_ENTITY).subscribe((resonse:C11_M_ENTITY[])=>{
      if(resonse[0])
