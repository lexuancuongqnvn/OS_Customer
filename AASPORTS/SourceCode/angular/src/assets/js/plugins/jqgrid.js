...
function conver_width(input, min_width) {
    ...
    input.forEach(element => {
        ...
        let w_int = parseInt(w - (w * padding));
        if (w_int < 100 && element.width >= 5) w_int = 100;
        else if (w_int < 50 && element.width < 5) w_int = 50;
        element.width = w_int;
    });
    return input;
}
...
function render_jqgrid_edit(data, col_model, table, footerrow = false, minHiehgt, rowNum = 20, isAdd = true, isEdit = true, isDel = true, isCopy = true) {
    ...
    if ($('.form-jqgrid-edit').width() == 0) {
        ...
        $(id.trim()).jqGrid({
            ...
            styleUI: 'Bootstrap',
            datatype: "local",
            colModel: conver_width_edit(col_model, table),
            viewrecords: true,
            height: minHiehgt,
            width: $('.form-jqgrid-edit').width(),
            //autowidth: true,
            shrinkToFit: false,
            footerrow: footerrow,
            ...
            loadComplete: function() {
                addNavButtonAdd(table, isAdd, isEdit, isDel, isCopy);
                var $this = $(this),
        ...
        if (col.sorttype) {
            switch (col.sorttype) {
                ...
                case 'select':
                    ...
                    $('.' + _id.replace('#', '') + '-basic-multiple').select2({
                        theme: "classic"
                    });
                    break;
                case 'multiselect':
            ...
            switch (col.sorttype) {
                case 'int':
                    ...
                    $(_id).on("change", function() {
                        ...
                        // if(col.editrules && col.editrules.required) {
                        //     $('input[name="'+col.name+'"]').addClass('required')
                        // }
                    checkType = true;
                    break;
                case 'checkbox':
                ...
                case 'select':
                    ...
                    $('.' + _id.replace('#', '') + '-basic-multiple').select2({
                        ...
                        matcher: function matchCustom(params, data) {
                            ...
                            if (_selectpickerGrigTimeout) clearTimeout(_selectpickerGrigTimeout);
                            _selectpickerGrigTimeout = setTimeout(() => {
                                console.log($.trim(params.term))
                                $('#value-autocomplated-search')[0].value = _id.replace('#', '') + '-basic-multiple[]' + $.trim(params.term);
                                $('#value-autocomplated-search')[0].click()
                            }, 500);
...
function conver_width_edit(input, table) {
    if ($('.form-jqgrid-edit').width() == 0) {
        setTimeout(() => {
            conver_width_edit(input, table);
        }, 500);
    } else {
...
function addNavButtonAdd(table, isAdd, isEdit, isDel, isCopy) {
    if (getCurrentForm() != EditPageState.viewDetail) {
        var a_add = ``,
            a_edit = ``,
            a_del = ``,
            a_copy = ``;
        if (isAdd) a_add = `<a style="padding: 5px 5px;" href="javascript:addRowData('` + table + `');"><span class="material-icons"> add </span>Thêm</a> `
        if (isEdit) a_edit = `<a style="padding: 5px 5px;" href="javascript:;"><span class="material-icons"> edit </span>Sửa</a>`
        if (isDel) a_del = `<a style="padding: 5px 5px;" href="javascript:deleteData('` + table + `');"><span class="material-icons"> delete </span>Xóa</a>`
        if (isCopy) a_copy = `<a style="padding: 5px 5px;" href="javascript:;"><span class="material-icons"> file_copy </span>Copy</a>`;

        $('#jqGridPager_' + table.trim() + '_left').html(`<div>
                ${a_add+a_edit+a_del+a_copy} 
            </div>`)
            // $('#jqGridPager_' + table.trim() + '_left').html(`<div>
